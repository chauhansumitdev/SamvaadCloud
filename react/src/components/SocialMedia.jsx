...
const MainContent = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await otherRoutes.home();
      const postsWithComments = await Promise.all(
        response.data.posts.reverse().map(async (user) => {
          const postsWithComments = await Promise.all(
            user.posts.map(async (post) => {
              const commentsResponse = await otherRoutes.getComments(post._id);
          ...
          <div key={user.username}>
            {user.posts.map((post) => (
              <div key={post._id} className="post">
                ...
                >
                  <input type="text" name="comment" placeholder="Add a comment" className='commentinput'/>
                  <button type="submit" className='commentbutton'><IoIosSend/></button>
                </form>
